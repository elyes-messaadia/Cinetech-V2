...
const MoviesPage = () => {
  ...
  useEffect(() => {
    const fetchGenres = async () => {
      try {
        const genresList = await tmdbService.getMovieGenres();
  ...
  // Charger les films avec filtres
  useEffect(() => {
          ...
          {loading ? (
            ...
            <div className="bg-background-light rounded-lg p-6 text-center">
              <p className="text-gray-400">
                Aucun film ne correspond à vos critères. Essayez de modifier vos filtres.
              </p>
            </div>
          ) : (
            <>
